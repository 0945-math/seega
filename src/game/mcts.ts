...
class MCTS {
    ...
    if (state.phase === 'placing') {
      ...
      for (let r = 0; r < BOARD_SIZE; r++) {
        for (let c = 0; c < BOARD_SIZE; c++) {
          ...
          const validMoves = getValidMoves(state.board, { row: r, col: c });
  ...
  private applyMove(state: GameState, move: { from?: Position; to: Position }): GameState {
    ...
    if (!move.from) {
      ...
      if (captures.length > 0) {
        ...
        newState.capturedBy = { ...state.capturedBy };
  ...
  private backpropagate(node: MCTSNode, value: number): void {
    ...
    while (current) {
      ...
      // 相手番では価値を反転
      if (current.state.currentPlayer !== node.state.currentPlayer) {
        value = -value;
      }
  ...
  search(state: GameState, numSimulations?: number): { from?: Position; to: Position } | null {
    ...
    for (let i = 0; i < simulations; i++) {
      ...
      if (!leaf.state.gameOver) {
        ...
        // シミュレーション
        const value = this.simulate(leaf.state, state.currentPlayer);
        
        // バックプロパゲーション
        this.backpropagate(leaf, value);
      } else {
        // 終了状態の価値
        const value = leaf.state.winner === state.currentPlayer ? 1 : -1;
        this.backpropagate(leaf, value);
      }
    }
