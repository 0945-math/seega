...
export function placePiece(state: GameState, pos: Position): GameState {
  ...
  if (allPlaced) {
    ...
    newMessage = `配置フェーズ：${nextPlayer === 'player1' ? 'あなた' : 'AI'}の駒を2個置いてください`;
  } else {
  ...
  return {
    ...
    lastMove: { to: pos },
    capturedPositions: [],
    turnNumber: shouldSwitchTurn ? state.turnNumber + 1 : state.turnNumber,
  };
}
...
export function movePiece(state: GameState, from: Position, to: Position): GameState {
  ...
  return {
    ...
    capturedPositions,
    consecutivePasses: 0,
    turnNumber: canContinueCapture ? state.turnNumber : state.turnNumber + 1,
  };
}
